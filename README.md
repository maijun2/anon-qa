...
- 質問一覧は「未回答 / 回答済み / 自分の質問 / すべて」で絞り込み、添付画像はクリックでページ内に拡大表示
...
4. `/admin/s/:id` で質問の確認・回答・回答済み管理、参考情報の共有、アンケートの配信を行う(アンケートは下書きの編集・既存アンケートからの複製が可能)
...
4. 質問一覧は「未回答 / 回答済み / 自分の質問 / すべて」の 4 タブで絞り込み(件数バッジ付き)。件数が増えても自分の質問や未回答を追いやすい
5. 添付画像はクリックするとページ内に拡大表示(Esc または背景クリックで閉じる)。⌘/Ctrl+クリックで従来どおり別タブでも開ける
6. 講師の回答・参考情報・アンケートはリアルタイムに反映される
